...
$(function(){
    ...
    $("button.deleteConfirmButton").click(function(){
     ...
     //bootstrap模态框，hidden.bs.modal事件表示当模态框完全对用户隐藏时触发。
    $('#deleteConfirmModal').on('hidden.bs.modal', function (e) {
        if(deleteOrderItem){
            var page="foreDeleteOI";
            $.post(
                    page,
    ...
    */
    $("button.createOrderButton").click(function(){
        ...
        location.href="foreConfirmOrder?"+params;
    });
...
function syncPrice(pid,num,price){
	//设置input元素 orderItemNumberSetting的值,表示数量
    $(".orderItemNumberSetting[pid="+pid+"]").val(num);
    //计算一个产品的总价(数量*单价),并设置文本
	var cartProductItemSmallSumPrice = formatMoney(num*price);
    $(".cartProductItemSmallSumPrice[pid="+pid+"]").html("￥"+cartProductItemSmallSumPrice);
  	//计算购物车总价、总件数
    calcCartSumPriceAndNumber();
    //ajax异步请求foreUpdateOI,通过控制器调用Dao层.
    var page = "foreUpdateOI";
    $.post(
            page,
            {"pid":pid,"number":num},
            function(result){
